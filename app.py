...
def stt():
    ...
    try:
        ...
        # Extracted Logic
        return process_transcription_results(segments_list, info)
...
def process_transcription_results(segments_list, info):
    ...
    for seg in segments_list:
        ...
        for w in source:
            ...
            # Weighted duration: small kanas (combining) get 0 weight, others 1.
            # Small tsu (っ/ッ) is a mora (silence), so it counts as 1.
            small_kana_set = set('ぁぃぅぇぉゃゅょゎァィゥェォャュョヮ')
            
            weights = []
            total_weight = 0
            for c in valid_chars:
                w_val = 0 if c in small_kana_set else 1
                weights.append(w_val)
                total_weight += w_val
            
            # Fallback: if all chars are small, treat them equally
            if total_weight == 0:
                total_weight = len(valid_chars)
                weights = [1] * len(valid_chars)

            unit_dur = duration / total_weight
            
            current_weight = 0
            for i, c in enumerate(valid_chars):
                seg_w = weights[i]
                c_start = w_start + (current_weight * unit_dur)
                current_weight += seg_w
                c_end = w_start + (current_weight * unit_dur)
